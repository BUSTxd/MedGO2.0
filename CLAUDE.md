...
**Íconos**: sin emojis. Registro de SVG line-art en `src/components/investigacion/Icono.tsx` (`<Icono name="..."/>`, `currentColor`, viewBox 24). El campo `icono` de cada `TarjetaContenido` y el `icono` de cada `Insignia` (`badges.ts`) son **claves** de ese registro (p. ej. `'microscopio'`, `'balanza'`, `'diana'`). Para un ícono nuevo, añadir una entrada al objeto `ICONOS` y usar su clave. Los SVG multicolor con degradados/filtros propios (estrella del HUD, copa, bola de cristal) se inline en su componente con **ids prefijados** para evitar colisiones. Las flechas `→`/`↔` dentro de textos son tipográficas, no íconos.
