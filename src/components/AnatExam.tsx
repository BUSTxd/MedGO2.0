'use client';
// Examen interactivo de identificación anatómica (motor compartido por EVA 2, EVA 3, …).
// Recibe su banco de preguntas por props; toda la lógica de flujo es común.
//
// Flujo por pregunta:
 ...
 *  o si contiene TODAS las palabras de `acceptAll` en cualquier posición. */
export type Concept = { label: string; accept: string[]; acceptAll?: string[] };
...
export type Question = {
  ...
  /** Cuándo revelar el overlay. 'solved' (defecto): al responder A. 'bChecked': al fallar A o tras verificar B. */
  overlayTrigger?: 'solved' | 'bChecked';
  /** Si true, la imagen base hace fade-out cuando el overlay aparece (útil cuando el overlay reemplaza la vista). Por defecto false: ambas imágenes se apilan. */
  overlayHideBase?: boolean;
  imageAlt?: string;         // imagen alternativa accesible mediante toggle tras responder A
  imageDarkBg?: boolean;     // fondo oscuro en el recuadro (útil cuando el overlay tiene trazos blancos)
  imageCaption?: string;     // descripción anatómica de la imagen
  imageCaptionList?: Array<string | { text: string; bold: true }>; // leyenda numerada
...
type AnatExamProps = {
  questions: Question[];
  /** Texto del kicker naranja, p. ej. "EVA 3 · Anatomía". */
  kicker: string;
...
function matchesConcept(input: string, c: Concept): boolean {
  if (matches(input, c.accept)) return true;
  if (c.acceptAll) {
    const n = norm(input);
    return c.acceptAll.every((w) => n.includes(norm(w)));
  }
  return false;
}
...
export default function AnatExam({ questions, kicker, title }: AnatExamProps) {
  ...
  const overlayReady = q.overlayTrigger === 'bChecked'
    ? (aWrong || (aCorrect && p.bChecked))
    : solved;
  const showingOverlay = !!q.imageOverlay && overlayReady && (overlayView[q.id] ?? true);
  ...
  const foundB = useMemo(
    () => q.conceptsB.map((c) => matchesConcept(p.bValue, c)),
    [q, p.bValue],
  );
  ...
  const preloadSrcs = useMemo(() => {
    ...
    const upto = Math.min(shuffled.length, idx + 1 + PRELOAD_AHEAD);
    for (let i = 0; i < upto; i++) {
      for (const s of [shuffled[i].image, shuffled[i].imageOverlay, shuffled[i].imageAlt]) {
        if (s) seen.add(s);
      }
    }
    return [...seen];
  ...
  return (
    <div className={base.examPage}>
      {/* Precarga progresiva (ventana deslizante). Renderizamos <Image> ocultas con
          la MISMA URL optimizada y el MISMO `sizes` que el render real, de modo que
          comparten la entrada de caché del navegador. Usamos loading="eager" (no
          priority) para descargarlas de inmediato sin competir en prioridad con la
          imagen visible, que sí lleva `priority`. */}
      <div aria-hidden style={{ position: 'absolute', width: 0, height: 0, overflow: 'hidden' }}>
        {preloadSrcs.map((src) => (
          <div key={src} style={{ position: 'relative', width: 1, height: 1 }}>
            <Image src={src} alt="" fill loading="eager" sizes="(max-width: 860px) 100vw, 460px" />
          </div>
        ))}
      </div>
      ...
      <div className={`${styles.body} ${showExplanation ? styles.bodyTop : ''}`}>
        ...
        <div className={styles.imageCol}>
          <div
            className={`${styles.imageBox} ${q.imageDarkBg && solved ? styles.imageBoxDarkSolved : ''}`}
          ...
          >
            {q.image && (
              <Image
                ...
                className={
                  q.imageAlt
                    ? `${styles.imageLayer} ${showingAlt ? styles.imageLayerHidden : styles.imageLayerVisible}`
                    : q.imageOverlay && q.overlayHideBase
                    ? `${styles.imageBaseTransition} ${showingOverlay ? styles.imageBaseHidden : ''}`
                    : undefined
                }
              />
            )}
            ...
            {q.imageOverlay && !showingAlt && (
              <Image
                ...
                sizes="(max-width: 860px) 100vw, 460px"
                style={{ objectFit: 'contain' }}
                className={`${styles.imageOverlay} ${showingOverlay ? styles.imageOverlayVisible : ''}`}
              />
            )}
            {!q.image && !q.imageAlt && (
          ...
          {overlayReady && q.imageOverlay && (
            <button
              className={styles.toggleImgBtn}
              onClick={() => setOverlayView((prev) => ({ ...prev, [q.id]: !(prev[q.id] ?? true) }))}
            >
              {showingOverlay ? '← Ver imagen original' : 'Ver imagen de referencia →'}
            </button>
          )}
...
function dotDone(progress: Record<string, QProgress>, qq: Question): boolean {
  ...
  return qq.conceptsB.filter((c) => matchesConcept(pp.bValue, c)).length >= need;
}
