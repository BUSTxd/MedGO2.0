...
import { semanas, curso, UNIDAD_COLOR, TIPO_BADGE } from '@/lib/data/cienciasSociales';
...
const UNIDAD_LABEL: Record<string, string> = {
  ...
  CONTEXTO:     'Contexto cultural y sociedad peruana',
  HISTORIA:     'Perspectiva histórica y política',
  EVALUACION:   'Evaluación',
};

export default async function CienciasSocialesPage() {
  ...
  return (
    <div className={styles.microPage}>
      ...
      <CienciasSocialesIcon size={160} className={styles.microPageIcon} />
      ...
      <div className={styles.container}>
        ...
        <div className={styles.formulaCard}>
          ...
          <div className={styles.formulaItems}>
            <span className={styles.formulaChip}><strong>Pasos 28%</strong> — Paso 1 (10%) + Paso 2 (18%)</span>
            <span className={styles.formulaChip}><strong>Cooperativos 20%</strong> — TC1 (8%) + TC2 (12%)</span>
            <span className={styles.formulaChip}><strong>Exposiciones y monografía 9%</strong></span>
            <span className={styles.formulaChip}><strong>Exámenes 43%</strong> — Parcial (18%) + Final (25%)</span>
            <span className={styles.formulaChip}>Mínimo aprobación: <strong>11.00</strong> · plagio = nota 0</span>
          </div>
        </div>
        ...
        {semanas.map((semana) => (
          <section key={semana.id} className={styles.weekSection}>
            ...
            {semana.actividades.map((act) => {
              ...
              const isLocked = !act.linkOverride && !hasAcceso;

              const href = act.linkOverride ?? `/dashboard/cursos/ciencias-sociales/${act.id}`;
