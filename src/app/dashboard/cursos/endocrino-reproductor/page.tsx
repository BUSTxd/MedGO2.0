...
import { semanas, curso, UNIDAD_COLOR, TIPO_BADGE } from '@/lib/data/endocrino';
...
const UNIDAD_LABEL: Record<string, string> = {
  ENDOCRINA:   'Fisiología endocrina e hipófisis',
  METABOLISMO: 'Metabolismo basal y energético',
  ESTRES:      'Respuesta endocrina al estrés',
  REPRODUCTOR: 'Sistema reproductor',
  ANATOMIA:    'Anatomía y disección',
  EVALUACION:  'Evaluación',
};

export default async function EndocrinoPage() {
  ...
  return (
    <div className={styles.microPage}>
      ...
      <EndocrineIcon size={160} className={styles.microPageIcon} />
      ...
      <div className={styles.container}>
        ...
        <div className={styles.courseHeader}>
          ...
          <div className={styles.courseMeta}>
            ...
            <span className={styles.metaDot} />
            <span className={styles.metaItem}>{curso.coordinadores.join(' · ')}</span>
          </div>
        </div>
        ...
        <div className={styles.formulaCard}>
          ...
          <div className={styles.formulaItems}>
            <span className={styles.formulaChip}><strong>Conocimientos 50%</strong> — Examen Final 60% + TBL 30% + Invertidas 10%</span>
            <span className={styles.formulaChip}><strong>Desempeño 50%</strong> — Anatomía 40% + Histología 30% + SGP 20% + Antropometría 10%</span>
            <span className={styles.formulaChip}>Mínimo aprobación: <strong>11.00</strong> en cada componente · Sustitutorio 8 ene, máx. <strong>11</strong></span>
          </div>
        </div>
        ...
        {semanas.map((semana) => (
          <section key={semana.id} className={styles.weekSection}>
            ...
            {semana.actividades.map((act) => {
              ...
              // Las prácticas (anatomía, histología y el taller) son libres.
              const isPractica =
                act.tipo === 'ANATOMIA' || act.tipo === 'HISTOLOGIA' || act.tipo === 'TALLER';
              ...
              const href = act.linkOverride ?? `/dashboard/cursos/endocrino-reproductor/${act.id}`;
